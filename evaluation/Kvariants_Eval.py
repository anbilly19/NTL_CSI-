...
class KVariantEval:
    ...
    def _risk_assessment_helper(self, cls, cls_type, experiment_class, exp_path):
        ...
        for i in range(num_repeat):
            ...
            # np.random.seed(i*i*11)
            # random.seed(i*i*11)
            # torch.manual_seed(i*i*11)
            # torch.cuda.manual_seed(i*i*11)
            # torch.cuda.manual_seed_all(i*i*11)
            dataset = load_data(self.data_name, cls, cls_type, best_config['norm'])
            val_auc, test_auc, test_ap,test_f1,scores,labels = experiment.run_test(dataset,logger,seed=int(i),seed_folder=self._NESTED_FOLDER)
