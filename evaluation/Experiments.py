...
class runExperiment():
    ...
    def run_test(self, dataset, logger,seed,seed_folder):
        train_data, val_data, test_data = dataset

        model_class = self.model_config.model
        loss_class = self.model_config.loss
        shift_loss_class = self.model_config.shift_loss
        ...
        model = model_class(network(),x_dim, config=self.model_config,seed=seed)
        ...
        checkpoint_path = os.path.join(seed_folder,f'model_{seed}.pth')
        checkpoint = os.path.exists(checkpoint_path)
        if checkpoint:
            model.load_state_dict(torch.load(checkpoint_path))
            trainer = trainer_class(model, loss_function=loss_class(self.model_config['loss_temp']),criterion=shift_loss_class(),
                         device=self.model_config['device'],lbd=self.model_config['lbd'])
            val_auc, val_ap,val_f1, valin_loss,valout_loss,_,_ = trainer.detect_outliers(val_loader,cls=None)
            test_auc, test_ap,test_f1, testin_loss,testout_loss,scores,labels = trainer.detect_outliers(test_loader,cls=None)
        if not checkpoint:
            trainer = trainer_class(model, loss_function=loss_class(self.model_config['loss_temp']),criterion=shift_loss_class(),
                         device=self.model_config['device'],lbd=self.model_config['lbd'])
