...
class ResNet(nn.Module):
    def __init__(self, channel,depth=3, hdim = 16,zdim=64,batchnorm=False,bias=False):
        ...
        self.res = nn.Sequential(*res_blocks)
        self.fc = nn.Linear(self.nChannels,zdim,bias=bias)
        self.shift_cls_layer = nn.Linear(self.nChannels, 2,bias=bias)
        self.relu = nn.ReLU(inplace=True)
        self.batchnorm = batchnorm
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        out = self.res(out)
        out_shift = self.shift_cls_layer(out.view(out.shape[0],-1))
...
class VisNets():
    ...
    def _make_nets(self,x_dim,config):
        ...
        trans = nn.ModuleList(
            [ImageTransformNet(x_dim, trans_nlayers) for _ in range(num_trans + neg_num)])
